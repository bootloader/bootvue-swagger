                ...
                <div class="card-header msg_head chat-head">
                    <div class="d-flex bd-highlight chat-header-left">
                        ...
                        <div class="user_info"
                            v-if="activeChat"
                            @click="MyFlags.showContactProfile = !MyFlags.showContactProfile">
                            <span class="user_name">{{contact}}</span>
                            <br/>
                            <span class="user_text">{{activeChat.startSessionStamp | formatDate}} - {{activeChat.closeSessionStamp | formatDate}}</span>
                        </div>
                        <div class="video_cam">
...
<div v-else-if="activeChat" v-for="m in activeChat.messages">
    ...
    <div v-if="m.sender == contact" class="d-flex justify-content-start mb-4 chat-bubble" :title="m.tags ? m.tags.categories : null" >
        <div class="msg_cotainer">
            <span>{{m.text | striphtml | newlines}}</span>
             <div v-if="m.attachments"> <span class="fa fa-photo"></span>&nbsp;{{m.template}} 
        ...
        <span class="msg_time"><span class="msg_user">{{m.sender}}</span>&nbsp;&nbsp;{{m.timestamp|formatDate}}</span>
    </div>

    <div v-else-if="m.sender == sender" 
        ...
        <div class="msg_cotainer_send">
            ...
            <span class="msg_time_send">{{m.timestamp|formatDate}}&nbsp;&nbsp;<span class="msg_user">{{m.sender}}</span></span>
        </div>
    </div>   
...
<script>
    ...
    export default {
        ...
        props: {
            session: Object,sender : String, contact : String
        },
        computed : {
          activeChat : function (argument) {
            return this.session;
          }
        },
        data: () => ({
        ...
        methods: {
            async loadMessages(){
                console.log("GetSessionChats...", this.session);
                this.isLoading = true;
